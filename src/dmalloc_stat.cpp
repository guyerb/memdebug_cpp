...
void dmalloc_stat::_agebucket_update(std::time_t now)
{
  ...
  for ( ; i > 0; i--) {
    ...
    if (src_ndx == _s_agebucket.size()) continue;	/* last bucket doesn't update */
...
*/
  ...
  ut.ut_check("current allocations", (unsigned)12, stat._s_curr_alloc);
  ...
  ut.ut_check("declined update", (unsigned)14, stat._s_declined_update);
  stat.alloc(8192, t_1);
  bytes += 8192;
  ...
  ut.ut_check("current allocated bytes", bytes, stat._s_curr_bytes);
  ut.ut_check("alltime allocations", (unsigned)14, stat._s_allt_alloc);
  ...
  ut.ut_check("current allocated bytes", bytes, stat._s_curr_bytes);
  ut.ut_check("alltime allocations", (unsigned)12, stat._s_curr_alloc);
